...
public class ItemRegistry {
    ...
    private final Map<ResourceLocation, DeferredItem<? extends Item>> items = new HashMap<>();
    private Map<DeferredItem<? extends Item>, TagKey<Item>[]> datagenTags;
    ...
    public Stream<DeferredItem<? extends Item>> allItems() {
        return items.values().stream();
    }

    @SafeVarargs
    public final <B extends Block> void register(String path, DeferredBlock<B> dBlock, TagKey<Item>... tags)
    {
        if(dBlock != null)
        {
            ResourceLocation id = C.mk(path);
            DeferredItem<BlockItem> dItem = C.ItemsRegistry.registerSimpleBlockItem(path, dBlock);
            //items.put(id, dItem);

            if (datagenTags != null && tags != null && tags.length > 0) datagenTags.put(dItem, tags);
        }
    }

    @SafeVarargs
    public final DeferredItem<Item> register(String path, Item.Properties props, TagKey<Item>... tags)
    {
        if(props != null)
        {
            ResourceLocation id = C.mk(path);
            DeferredItem<Item> dItem = C.ItemsRegistry.registerSimpleItem(path, props);
            items.put(id, dItem);
